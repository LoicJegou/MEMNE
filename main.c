...
for (i=2;i<=ii-2;i+=2)
  {
  u[i][1]=(5.0*u[i][0]+15.0*u[i][2]-5.0*u[i][4]+u[i][6])*0.0625;

  for (j=3;j<=jj-3;j+=2)
    u[i][j]=(-u[i][j-3]+9.0*u[i][j-1]+9.0*u[i][j+1]-u[i][j+3])*0.0625;

  u[i][jj-1]=(5.0*u[i][jj]+15.0*u[i][jj-2]-5.0*u[i][jj-4]+u[i][jj-6])*0.0625;
  }
